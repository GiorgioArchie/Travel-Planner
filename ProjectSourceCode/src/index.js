...
document.addEventListener('DOMContentLoaded', () => {
  // Access the calendar container from the DOM
  const container = document.getElementById('calendar');

  // Options object for the Toast UI Calendar
  const options = {
    defaultView: 'week', // could also be 'day', 'month', etc.
    timezone: {
      zones: [
        {
  ...
  // Use the global `tui.Calendar` constructor from the CDN
  const calendar = new tui.Calendar(container, options);

  // Optionally, you can add other methods and event listeners here
  // For example:
